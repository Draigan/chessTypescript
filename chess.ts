...
let leftTradeSquare: SquareFace;
let rightOfPawn;
let rightStartingSquare;
let rightFindTradeSquare;
let rightTradeSquare: SquareFace;
let InitialDivSquare: SquareFace;
let aboveOrBelowInitalSquare: SquareFace;
let departureSquare: SquareFace;
...
interface SquareFace {
        ...
        col: number;
        possible: boolean;    // marks the square for movement possible
        encounter: boolean;  // marks the square for trade possible
        blackCount: number;        //represents pieces line of sight   
        whiteCount: number;
        pieceType: string;
        pieceColor: string; 
        altPieceType: string;     //alt board state for moving around pieces without messing up mainboard
        altPieceColor: string;
        pastPieceType: string;        //board state 1 move in the past
        pastPieceColor: string;
        enPassant: boolean;
}
...
class Square implements SquareFace { 
    ...
    constructor(_rowParam: number, _colParam: number, _typeParam: string, _colorParam: string){
...
findKing(color: string) { 
 if (squaresArray.find( square => square.pieceType == 'king' && square.pieceColor == color) != null) {
...
function
findSquare(rowinput: number, colinput: number): SquareFace | null {
...
function
turnColorSwitcher(){    currentTurnColor = currentTurnColor == 'white' ? 'black':'white'}
...
if (squaresArray[i].pieceType == 'bishop' && squaresArray[i].pieceColor == currentTurnColor){
 ...
 const checkMateElement = document.getElementById('checkmate-id');
  if (checkMateElement){
        if (true == checkMate) {return checkMateElement.innerHTML = 'CHECKMATE!!!!!!';
 ...
 function
mateDirection(row: number, col: number, direction: string,startSquare: SquareFace){
...
mainEventFunction(currentSquare){
    ...
    if (rightTradeSquare == currentSquare && currentTurnColor == 'black'){  enPassantMove(currentSquare.row,currentSquare.col,"blackleft"); ; changeTurn();  return}
    if (leftTradeSquare == currentSquare && currentTurnColor == 'black'){  enPassantMove(currentSquare.row,currentSquare.col,"blackright"); changeTurn();  return}

    if (rightTradeSquare == currentSquare && currentTurnColor == 'white'){  enPassantMove(currentSquare.row,currentSquare.col,"whiteright");  changeTurn();  return}
    if (leftTradeSquare == currentSquare && currentTurnColor == 'white'){  enPassantMove(currentSquare.row,currentSquare.col,"whiteleft");  changeTurn();  return}
